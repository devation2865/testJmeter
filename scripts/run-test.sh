...
check_services() {
    ...
    if ! docker ps | grep -q "scaling-app"; then
        print_message "❌ 應用服務未運行，請先啟動架構" $RED
        print_message "   運行: docker-compose up -d" $YELLOW
        exit 1
    fi
...
# 主程序
main() {
    print_message "🔧 JMeter Scaling 架構測試工具" $BLUE
    echo "=================================="
    ...
    case "${1:-}" in
        -h|--help)
            ...
            ;;
        -s|--single)
            run_single_test "single_server" 100 300 30
            ;;
        -m|--multi)
            print_message "🔍 檢查多服務器配置..." $BLUE
            local app_count=$(docker ps | grep -c "scaling-app" || echo "0")
            if [ "$app_count" -lt 2 ]; then
                print_message "⚠️  檢測到少於2個應用實例，建議先擴展服務器" $YELLOW
                print_message "   運行: docker-compose up -d --scale app=3" $YELLOW
            ...
            run_single_test "multi_server" 100 300 30
            ;;
        -c|--custom)
            if [ $# -lt 4 ]; then
            ...
            run_single_test "custom_test" "$2" "$3" "$4"
            ;;
        -a|--all)
            print_message "🔄 運行所有測試場景..." $BLUE
            run_single_test "single_server" 100 300 30
            run_single_test "multi_server" 100 300 30
            run_single_test "high_load" 200 600 60
            ;;
        "")
